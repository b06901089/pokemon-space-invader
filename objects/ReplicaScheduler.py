...
class ReplicaScheduler(pygame.sprite.Sprite):
    ...
    def __init__(self, sprite_groups, sprite, x, y, ani, idx, num_replicas, replica_delay_frames, spawn_dir, spawn_ops, spawn_reset):
        """
        Args:
    ...
    def update(self):
        ...
        if self.frame_counter >= self.replica_delay:
            if self.spawn_reset and self.sprite:
                from scripts.math_utils import get_animation_spawn_location
                self.x, self.y = get_animation_spawn_location(self.sprite_groups, self.sprite, self.spawn_dir, self.spawn_ops)
            if self.spawn_ops:
